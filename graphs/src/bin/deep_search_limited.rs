...
fn get_route(node: Box<Node>, objective: &'static str,mut limit: u8,mut t: u8) {
    ...
    for action in actions {
        ...
        if  !EXPLORED.lock().unwrap().contains(state) {
            get_route(Box::new(root_node.expand(action, state)), objective, 10,t);
        }
    }
...
fn main() {
    ...
    get_route(Box::new(root_node), objective, 10,1);
}
